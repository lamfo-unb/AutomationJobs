...
library(readxl)
isco88_isco08 <- read_excel("./Data/ISCO88_ISCO08.xlsx",
                            col_types = "text")

isco08_soc <- read_excel("./Data/ISCO08_SOC.xls", 
                         skip = 6, col_types = "text")

Job_Zones <- read_excel("./Data/All_Job_Zones.xls", 
...
conta <- cbo2002_soc %>% group_by(CBO2002) %>% summarise(count = n())

table(conta$count)

write.csv2(cbo2002_soc, row.names = F, file = "./Data/CBO2002_SOC.csv")
